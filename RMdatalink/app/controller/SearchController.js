...
Ext.define('RMdatalink.controller.SearchController', {
    ...
    oncustomSearchApplyBtnTap: function(button, e, eOpts) {
        Ext.Msg.alert("Alert ","Devlopment in progress",Ext.emptyFn);

        return ;


               var datalink_status = Ext.ComponentQuery.query('#rtDatalinkStatusSelFld')[0].getValue();
               var ecatalog_status =Ext.ComponentQuery.query('#rtEcatalogStatusSelFld')[0].getValue();
               var ecommerce_status =Ext.ComponentQuery.query('#rtECommerceStatusSelFld')[0].getValue();
               var rmpro_status =Ext.ComponentQuery.query('#rtRM_proStatusSelFld')[0].getValue();
               var irugs_status = Ext.ComponentQuery.query('#rtirugzStatusSelFld')[0].getValue();

        var searchByStatus = [] ;

        if(datalink_status && datalink_status != "SKIP"){

            searchByStatus.push({store_products:{"datalink_status":datalink_status}}) ;

        }

        if(ecatalog_status && ecatalog_status != "SKIP"){

            searchByStatus.push({store_products:{"ecatalog_status":ecatalog_status}}) ;

        }
        if(ecommerce_status && ecommerce_status != "SKIP"){

            searchByStatus.push({store_products:{"ecommerce_status":ecommerce_status}}) ;

        }

        if(rmpro_status && rmpro_status != "SKIP"){

            searchByStatus.push({store_products:{"rmpro_status":rmpro_status}}) ;

        }
        if(irugs_status && irugs_status != "SKIP"){

            searchByStatus.push({store_products:{"irugs_status":irugs_status}}) ;

        }

        this.searchRetailerByStatus("","ACTIVE",1,searchByStatus) ;
    },
