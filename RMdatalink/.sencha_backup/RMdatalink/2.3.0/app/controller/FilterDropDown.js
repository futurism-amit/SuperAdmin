...
Ext.define('RMdatalink.controller.FilterDropDown', {
    ...
    onFilterWithData: function(sheet, filterData) {
        ...
        if(filterData.due_in) {

            filterData.due_in = parseInt(filterData.due_in) ;
            if(filterData.due_in>0) {

                var currentDate = new Date();
                var createdInPastFilter = new Ext.util.Filter({
                    filterFn: function(item) {
                        var lastCreatedDate = new Date( getDueDate(item.data));
                        var differenceInDays = Ext.Date.diff(currentDate,lastCreatedDate, Ext.Date.DAY);

                        if(differenceInDays <= filterData.due_in)
                            return true;
                        return false;
                    }
                });
